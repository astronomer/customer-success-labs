...
def all_logs(dbname, ds, incremental=False):
    if incremental is False:
        sql = f"""select '{dbname}' as db_name, * from {dbname}_log"""
    else:
        sql = f"""select '{dbname}' as db_name, * from {dbname}_log where dttm::date = '{ds}'::date"""
    return sql

def all_taskinstances(dbname, ds, incremental=False):
    if incremental is False:
        sql = f"""select '{dbname}' as db_name, * from {dbname}_ti"""
    else:
        sql = f"""select '{dbname}' as db_name, * from {dbname}_ti where start_date::date = '{ds}'::date"""
    return sql